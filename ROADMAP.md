...
2. 🔄 **Day 6 — pipeline done, number invalid.** `icp_odometry` runs at 28 Hz off the depth cloud with loop closure structurally off, scored against PX4 ground truth — but the aircraft left the commanded square at 16.4 m/s and the estimator tracked 47% of the distance, so the drift figure is not a benchmark (`VIO.md` §3). Chase the flight anomaly, give ICP a motion prior, then re-measure. Front-end **installed: `rtabmap_ros`**. Run it in the forest world with loop closure OFF, scored against PX4 ground truth. Note this measures *plumbing*, not the hard part — the sim's camera-IMU extrinsic is exact and known, whereas on hardware calibrating it is the actual work. That is why the offline OpenVINS/EuRoC ladder (item 6) stays the real deliverable.
